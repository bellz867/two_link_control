...
if __name__ == '__main__':
    ...
    l = 20
    L = l+1
    dyn = dynamics.Dynamics(alpha=alpha,betar=betar,betadel=betadel,gammath=gammath,gammaw=gammaw,gammav=gammav,tauN=tauNoise,phiN=phiNoise,phiDN=phiDNoise,phiDDN=phiDDNoise,l=l,useNN=useNN,useYth=useYth,useBias=useBias,useCuda=useCuda)
    ...
    VHHist = usecuda(torch.zeros((5*l,len(t)),dtype=torch.float),useCuda=useCuda)
    ...
    #start save file
    savePath = "C:/Users/bell_/OneDrive/Documents/_teaching/AdaptiveControlSpring2022/projects/project4/two_link_two_layer"
    ...
    for jj in range(0,len(t)):
        ...
        VHHist[:,jj] = torch.reshape(VHj,(5*l,))
        tauHist[:,jj] = tauj
        tauffHist[:,jj] = tauffj
        taufbHist[:,jj] = taufbj
        fHist[:,jj] = fj
    ...
    for ii in range(5*l):
        VHax.plot(t,VHHist[ii,:],color=rand(3).numpy(),linewidth=2,linestyle='--')
